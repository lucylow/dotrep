...
class Helm {
  ...
  async installDeps(index=0) {
    ...
    // Install dotrep if enabled
    if (this.config.dotrep && this.config.dotrep.enabled) {
      await this._installDotrep(index);
    }
  }
  ...
  async _installDotrep(index) {
    const dotrepConfig = this.config.dotrep;
    // Get the project root (two levels up from config directory)
    const projectRoot = path.join(this.files.projectConfigPath(), '..');
    const dotrepChartPath = path.join(projectRoot, 'dotrep-v2', 'helm');
    
    // Determine resources: use dotrep-specific resources, fall back to top-level resources, then defaults
    let resources = dotrepConfig.resources;
    if (!resources && this.config.resources) {
      resources = this.config.resources;
    }
    if (!resources) {
      resources = {
    ...
    // Build ingress annotations for cert-manager if enabled
    const ingressAnnotations = {};
    if (dotrepConfig.ingress?.tls?.certManager?.enabled) {
      ingressAnnotations['cert-manager.io/issuer'] = dotrepConfig.ingress.tls.certManager.issuer || 'letsencrypt-prod';
      ingressAnnotations['kubernetes.io/tls-acme'] = 'true';
    }

    // Build autoscaling config with all required fields
    const autoscalingConfig = dotrepConfig.autoscaling || {
      enabled: false,
      minReplicas: 1,
      maxReplicas: 3
    };
    // Ensure autoscaling has target utilization percentages if enabled
    if (autoscalingConfig.enabled) {
      if (!autoscalingConfig.targetCPUUtilizationPercentage) {
        autoscalingConfig.targetCPUUtilizationPercentage = 70;
      }
      if (!autoscalingConfig.targetMemoryUtilizationPercentage) {
        autoscalingConfig.targetMemoryUtilizationPercentage = 80;
      }
    }

    // Format health check probes properly
    const formatHealthCheck = (healthCheck) => {
      if (!healthCheck) return null;
      const path = healthCheck.path || (healthCheck.httpGet?.path) || '/health';
      // Ensure port is a number (Kubernetes requires integer for httpGet.port)
      const port = parseInt(healthCheck.port || healthCheck.httpGet?.port || 3000, 10);
      return {
        httpGet: {
          path: path,
          port: port
    ...
    // Generate values for dotrep
    const values = {
      replicaCount: dotrepConfig.replicas || 1,
      image: {
        repository: dotrepConfig.image?.repository || 'dotrep-v2',
        tag: dotrepConfig.image?.tag || 'latest',
      ...
      ingress: {
        enabled: dotrepConfig.ingress?.enabled || false,
        className: 'nginx',
        annotations: ingressAnnotations,
        hosts: dotrepConfig.ingress?.host ? [{
          host: dotrepConfig.ingress.host,
          paths: [{ path: '/', pathType: 'Prefix' }]
        }] : [],
        tls: dotrepConfig.ingress?.tls?.enabled ? [{
          secretName: `dotrep-tls-${this.config.name}`,
          hosts: dotrepConfig.ingress.host ? [dotrepConfig.ingress.host] : []
        }] : []
      },
      resources: resources,
      autoscaling: autoscalingConfig,
      env: dotrepConfig.env || {},
      livenessProbe: formatHealthCheck(dotrepConfig.healthChecks?.liveness) || {
      ...
      readinessProbe: formatHealthCheck(dotrepConfig.healthChecks?.readiness) || {
    ...
    if (values.env) {
      Object.keys(values.env).forEach(key => {
        ...
        if (typeof value === 'string' && value.startsWith('${') && value.endsWith('}')) {
          ...
          values.env[key] = process.env[envVar] || value;
        }
      });
    }

    // Add polkadot configuration as environment variable if present
    // This is added after substitution to avoid accidental substitution of JSON content
    if (dotrepConfig.polkadot) {
      values.env.POLKADOT_CONFIG = JSON.stringify(dotrepConfig.polkadot);
    }

    const relativeValuesFilePath = this.files.relativeValuesFilePath(this.config.name, 'dotrep');
    const fullValuesFilePath = this.files.absoluteValuesFilePath(this.config.name, 'dotrep');
    this.files.writeYAML(fullValuesFilePath, values);

    // Install dotrep using local helm chart
    const cmd = `upgrade --install dotrep-v2 -f ${relativeValuesFilePath} --namespace ${this.namespace} ${dotrepChartPath}`;
    console.log(cmd);
    try {
      await this._cmd(cmd);
    } catch (e) {
      console.log(`Could not install dotrep-v2: ${e.message}`);
      throw e;
    }
  }
