...
class Files {
  constructor(config={}) {
    ...
    if(!this.config.dataPath) {
      this.config.dataPath = strings.removePathSpaces(ospath.data());
    } else {
  ...
  deleteDeploymentDirectories(name) {
    ...
    if (this.config.remote && this.config.remote.clusters) {
      for (let n = 0; n < this.config.remote.clusters.length; n++){
        this.deleteDirectory(`${deploymentPath}-${n}`);
      }
    }
  }
  readJSON(filePath) {
  ...
  terraformTplPath(type) {
    // Use __dirname for more reliable path resolution in cloud deployments
    return path.join(__dirname, '..', '..', 'terraform', type);
  }
  copy (source, target) {
  ...
  copyChartTemplates(name) {
    const templatesDirName = 'templates';

    const source = path.join(this.projectConfigPath(), 'charts', templatesDirName);
    const target = path.join(this.valuesPath(name), templatesDirName);
