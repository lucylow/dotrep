...
class RemoteCluster {
  ...
  async create() {
    ...
    for (let counter = 0; counter < this.config.remote.clusters.length; counter++) {
      const name = nameLib.generate(this.config.name, counter);
      // give one more worker for additional services
      ...
      creationPromises.push(new Promise(async (resolve) => {
        ...
        await this._saveKubeconfig(counter, name, false);
        resolve(true);
      }));
    }
  ...
  async installNodes() {
    ...
    for (let counter = 0; counter < this.config.remote.clusters.length; counter++) {
      let startNode = 0
      let endNode = partition[0] - 1;
      for (let index = 0; index < counter; index++) {
        startNode += partition[index];
        endNode += partition[index + 1];
      }

      this.helm[counter].setNodes(partition[counter]);

      const startKeyIndex = startNode - (counter * nonValidatorsPerCluster);
      const endKeyIndex = endNode - (nonValidatorsPerCluster * (counter + 1));

      const sessionTypes = crypto.sessionTypes();
      const sessionKeys = {};
      sessionTypes.forEach((type) => {
        sessionKeys[type] = this.config.keys[type].slice(startKeyIndex, endKeyIndex + 1);
      });
      this.helm[counter].setSessionKeys(sessionKeys);

      const nodeKeys = this.config.nodeKeys.slice(startNode, endNode + 1);
  ...
  _initializeTerraform(index) {
    ...
    const gcloudPath = this._getGCloudPath();
    ...
    ['main.tf', 'provider.tf', 'variables.tf', 'outputs.tf', 'backend.tf', 'remote-state/main.tf'].forEach((item) => {
      ...
      const awsAccountId = this.config.remote.awsAccountID || constants.defaultAWSAccountId;
  ...
  _stateStoreOptions() {
    ...
    // Path to terraform binary in parent directory (used as prefix: ../terraform)
    // This is used in shell commands, so '../' is correct for cloud/Linux deployments
    const terraformPath = '../';
  ...
  _getGCloudPath() {
    return this.config.gcloudPath || constants.defaultGCloudPath;
  }
  ...
  _validatorBootnodes(clusterIndex, nodesPerCluster, nonValidatorsPerCluster) {
    ...
    const port = constants.initialP2PPort + nodesPerCluster - nonValidatorsPerCluster;

    const validatorNodeIndex = validatorBootnodeCfg.clusterIndex * nodesPerCluster + nodesPerCluster - nonValidatorsPerCluster;
  ...
  async _saveKubeconfig(counter, name, init=true) {
    if (init) {
      await this._cmd('init', counter);
    }
    ...
    this.files.write(targetKubeconfigPath, kubeconfigContent, {mode: 0o600});
  }
}
